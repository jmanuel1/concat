...
class TestSubVisitors(unittest.TestCase):
    ...
    def test_try_word_visitor(self) -> None:
        token = concat.level0.lex.Token()
        token.start = (0, 0)
        node = concat.level1.parse.TryWordNode(token)
    ...
    def test_with_word_visitor(self) -> None:
        token = concat.level0.lex.Token()
        token.start = (0, 0)
        node = concat.level1.parse.WithWordNode(token)
