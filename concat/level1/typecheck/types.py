...
class ObjectType(IndividualType):
    ...
    def constrain(self, supertype: Type, constraints: Constraints) -> None:
        ...
        if self._arity != supertype._arity:
            raise TypeError('{} and {} do not have the same arity'.format(self, supertype))
        for name, type in self._attributes.items():
            type.constrain(supertype._attributes[name], constraints)
        for self_param, super_param in zip(self._type_parameters, supertype._type_parameters):
